...
def handle_photo(update: Update, context: CallbackContext) -> None:
    ...
    if update.message.photo:
        ...
        update.message.reply_text('No photo found in the message.')
    ...
    if update.message.document:
        ...
        if mime_type in ['image/gif', 'video/mp4']:
            ...
            if mime_type == 'image/gif':
                processed_gif_path = process_gif(media_path, session_id, update.message.from_user.id)
                context.bot.send_animation(chat_id=update.message.chat_id, animation=open(processed_gif_path, 'rb'))
            elif mime_type == 'video/mp4':
                processed_video_path = process_video(media_path, session_id, update.message.from_user.id)
